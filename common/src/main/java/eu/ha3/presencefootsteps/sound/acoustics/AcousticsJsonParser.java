...
public class AcousticsJsonParser {
    ...
    private void doParse(Reader reader) throws JsonParseException {
        ...
        if (json.get("engineversion").getAsInt() != ENGINE_VERSION) {
            throw new JsonParseException("Unrecognised Engine version: " + ENGINE_VERSION + " expected, got "
        ...
        json.getAsJsonObject("contents").entrySet()
                .forEach(element -> lib.addAcoustic(element.getKey(), solveAcoustic(element.getValue(),
                        "events")));
    }
